...
**Answer:** 131,395 observations and 24 variables
...
<chr> or character data type; I'm guessing that the last name column represents each voter's surname, while the amount column represents how many U.S. dollars each person donated.
...
In the space below, write the first and last name from the *second* row of the data.

**Answer:** Peggy and Mukai
...
**Answer:** A significant event happened in real life (Donald Trump used fear and anger to prompt his voters to donate) and the contributions more than doubled.
...
**Answer:** We told R to summarize the data by counting how much each contribution was, allowing us to see how many donors paid such-and-such amount.
...
**Answer:** Now we've told R to count how many contributions went to which exact Republican support fund by summing each fund and counting the contributions.
...
**Answer:** We've added this piece of code: mean_amount = mean(amount),median_amount = median(amount), which gives us the mean and median of contributions given to each fund.
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **Answer** We've grouped the data by city, now, and added min_amount = min(amount), max_amount = max(amount), which allows us to see new information - the minimum and maximum payments that contributors in certain cities donated - as well as the total contributions, their means and medians.
