...
A bar chart in black and white, ordered alphabetically.
...
NOTE: use plus sign instead of pipe operator
NOTE: weight in this case means value - essentially the y value
...
Now the chart is in order of smallest number to largest number, creating a slope.
...
The data has now been flipped to the y-axis.
```{r}
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=reorder(County,UNAF_DIFF), weight=UNAF_DIFF)) +
...
It got rid of the space between the county names and their corresponding bars.
...
It is now a hideous blue color.
...
```
NOTE: the labs function makes labels
NOTE: MAKE THE TITLE OF THE CHART THE NEWS; THE HEADLINE
...
a number changing over time - perfect for a line chart.
...
A line chart with jagged peaks and valleys.

NOTE: x AND y axis cause it's a line chart; the chart helps us to see the variations in a way that just looking at the data would not
...
Now we have ticks that represent each week.
...
Scale x date changes the scale of the data, changing where the ticks go
...
Now the text is turned on a 45-degree angle, making it less squished and more aesthetically pleasing.
