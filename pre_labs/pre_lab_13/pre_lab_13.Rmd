...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**Answer**
I don't notice the names getting worse exactly, but they are getting less mongoose-themed.

```{r}
groq_response <- axolotr::ask(
  prompt = "Give me 20 names for a pet mongoose",
...
I guess you're getting a mongoose?
...
**Answer** Yes; Llama understood and spit out the correct information.
...
Yes; I would change the prompt to be a bit broader so that it pulls job requirements and qualifications.
...
**Answer** Yes, it correctly identifies the error - but the response changes slightly when run multiple times.
...
The output makes sense and lots of its suggestions are things we've been doing with this data all semester. The suggestions don't necessarily match the columns we have, though.
...
No! This was its response: "I'm happy to help you with your request. However, I need to clarify that I'm a large language model, I don't have the capability to visually see or access images. I can only respond based on the description provided.

Unfortunately, I don't have any information about the image you're referring to. If you can provide more context or a detailed description of the image, I'd be happy to try and assist you in identifying the specific things in the image." 
