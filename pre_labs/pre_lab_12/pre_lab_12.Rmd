...
The data frame consists of 999 of Cardin's press releases, with date, title, url and text columns.
```{r}
...
It shortens the original list by getting rid of repeat words.
...
It gives us a table with every single word in the press releases.
...
Not really - it's a bunch of articles at the top. Which is not surprising or fun.
...
It is more interesting than before - these words do actually mean something, but they don't actually appear that often.
```{r}
unique_words |>
...
A lot of the most frequently used words are the same, but "health" has moved down and "Baltimore" has taken the spot of most-used word in Cardin's 2024 releases.
...
Now we have a data frame with 2-word phrases that are more nuanced and likely include health-related words high up because of the pandemic.

```{r}

#Assembling, checking each word, then reassembling
releases |>
  filter(year(date) == 2022) |>
...
We've made the same kind of data frame as the previous question, but for 2024's releases. Health care is now the number one most used phrase, but in 2024 he seems to mention "amounts" a lot.
...
Lemmatization assumes that all these words are essentially the same. Stemming is important for if specificity is really the goal. 
...
Yes, lots of them could either be positive or negative fully depending on the context.
