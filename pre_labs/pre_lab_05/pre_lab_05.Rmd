...
**Answer**
Yes; 25 observations of 8 variables.
...
NOTE: the names need to be exactly the same when using join or it won't work.
...
```
NOTE: the order of data frames matters above because we're using a left join.
NOTE: |> nrow() spits out the number of rows.
...
Is it saying that the new frame has been joined by matching the county columns?

```{r}
maryland_population_with_voters <- maryland_population |> left_join(county_voters_2020)
```
NOTE: remember to give the new data frame a useful name
...
```
NOTE: GEO code also known as a FIPS code (federal information processing system)
...
NOTE: and it's specified by this:(COUNTY==NAME)
NOTE: the order is very important!!
NOTE: do the join first, before saving it to a dataframe, so you can check to make sure it's correct
...
Prince George's, which is 78% democratic
...
Howard County, which is 52% democratic
...
NOTE: never open zip code data in excel bc it will erase important zeroes at the beginning or end of the zip code.
...
now it's chr, or character data.
```{r}
