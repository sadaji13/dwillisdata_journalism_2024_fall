...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: 402,345; this implies that 
...
Deceased - you can't count them
Serving time for a felony conviction - depending on where they live, they might not be able to vote
Duplicate - why is there duplicated data?
Request from voter - interesting because why would someone request to be removed?
```{r}
...
**Answer**
There's a random 1- in the first row, some names of columns are uppercase and some lowerscase, one column has a random space in it
...
**Answer**
Made all the column names lowercase, eliminated numbers in columns named with a number firstand, changed spaces to underscores
```{r}
# cleaning function
...
You can't use the sum function on a character column
...
There are two duplicate rows; the voter name is Derrick
...
Now all the zip codes are the same and there aren't any zip codes that appear to be a range.
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
...
Just one. Also, what does this ~ symbol mean?
```{r}
# cleaning function
cleaned_conowingo <- conowingo |>
  clean_names() |> 
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many misspellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook.

**Answer**

Zero misspellings remain. But how did it know to correct "Conowingoo"? In previous code it seems like we told R what the misspelling was and explicitly what to correct it to.
Does this code, "str_detect(city,"^Conowing") ~ "Conowingo"" mean something like, "In the city column, detect anything with "Conowing" in it and make it "Conowingo.""?
