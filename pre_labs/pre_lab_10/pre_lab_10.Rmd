...
It contains normal data frame stuff but also contains geometric information.
...
A map with latitude and longitude lines and also ghost precincts.
```{r}
pg_precincts |>
  ggplot() +
  geom_sf() +
...
A data frame with over 3,000 rows and 18 columns including one for geometric data.
...
We've made a data frame with only the Maryland counties.
...
It's a map of Maryland without the bay.
...
A map of PG County divided by precincts.
...
ggplot() +
  ...
  theme_minimal()
```
#The thing you want to be most visible needs to go last; base layer goes first ! More detail on! top!
...
The output is a data frame showing precincts, early and absentee ballots, provisional and total eligible ballots as well as the total.
...
**Task** Run the following code. Describe the output in the space below. **Answer**
A map of PG County divided by precinct including "ghost precincts"
```{r}
...
Our map now has a legend and the precincts are in navy hues.
...
A data frame with voters and party affiliations across MD counties.
```{r}
...
#Running geometry = TRUE will take our data and give us a geometry column, making a map
