...
```{r echo=FALSE}
census_api_key("102c0c72d8f8eb880608f9dacb53fdf9100f8ba0", install=TRUE, overwrite = TRUE)
```
...
The `name` variable is the Census code for each k
ind of data, while `label` describes it and `concept` is a broader category.
...
It might be, depending on what the numbers actually are; the margins of error are too large to say for sure.

Is the difference between Howard and everyone else significant?

**Answer**

Yes - no other county is even close, regardless of the margin of error.
...
**Answer**
No; every county saw an increase.
