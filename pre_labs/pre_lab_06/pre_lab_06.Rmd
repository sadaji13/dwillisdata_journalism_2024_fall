...
It thinks the data is the column names; there's data outside of the data set.
...
R is now putting placeholder column names which aren't accurate.
...
NOTE: col_names = FALSE is telling R that there are NO column names here.
...
**Answer** Some columns still need to be more accurately named.
...
NOTE: using the separate function to make separate candidate and party columns, then using case_when to distinguish senate vs house columns, then putting office columns first, and "everything" else after it
