...
R sorts data by case, as well - it doesn't know that BALTIMORE and Baltimore are the same city

takes a stringed value and uppercases it so that R reads them as the same

`readr` anticipates some date formatting and can automatically handle many of these issues (indeed it uses lubridate under the hood). When you are importing a CSV file, be sure to use `read_csv`, not `read.csv`.
...
Be careful using the select function - can accidentally get rid of some columns
