...
sent_and_returned <- read_csv("data/md_absentee_ballots.csv")
  sent_and_returned_by_date <-read_csv("data/md_absentee_ballots_date.csv") 
...
```{r}
highest_percentages <- sent_and_returned|>
  mutate(percentages =
    (`TOTAL RECEIVED`/`TOTAL SENT`)*100)|>
  arrange(desc(percentages))|>
  head(10)
highest_percentages|>ggplot()+ geom_bar(aes(x=`COUNTY NAME`, weight=percentages))+coord_flip()+
  labs(
    title="Counties who Returned the Most Ballots",
    y = "Counties",
    x = "% of Returned Ballots",
...
```{r}
Dems_returned_ballots <- sent_and_returned|>
  mutate(
    percentages = (`DEM RECEIVED`/DEM_SENT)*100
  )|>select(`COUNTY NAME`,percentages)|>arrange(percentages)|>head(10)

Dems_returned_ballots|>ggplot()+ geom_bar(aes(x=`COUNTY NAME`, weight=percentages))+coord_flip()+
  labs(
    title="Counties who Returned the Most Democratic Ballots",
    y = "Counties",
    x = "% of Returned Ballots",
    caption = "source = Maryland State Board of Elections")
...
```{r}
frederick_received_ballots <- sent_and_returned_by_date|> filter(`COUNTY NAME` =="FREDERICK")

frederick_received_ballots |>
  ggplot() + 
  geom_line(aes(x=DATE, y=`TOTAL RECEIVED`)) + 
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d") +
  theme(
    axis.text.x = element_text(angle = 45,  hjust=1))+
  labs(
    title="Frederick County's Received Ballots",
    y = "No. of Ballots",
    x = "Date",
    caption = "source: Maryland State Board of Elections")
...
sent_and_returned|>
ggplot()+
  geom_point(aes(x=DEM_SENT, y= `DEM RECEIVED`
))
