...
**A1.**
https://datawrapper.dwcdn.net/52nlU/1/; 

I think the story is in the rise of mail-in voting due to the pandemic. I'm curious to see if it will taper off even more for future elections.
...
```{r}
joined_early_parties<- ev_totals_dem|>
  
  inner_join(ev_totals_rep, by = "Day") |>
  inner_join(ev_totals_una, by = "Day")

joined_early_parties
...
```{r}
most_populated_counties <- ev_totals_dem|>
  filter(Day==`Baltimore County`
  )
```


Okay, so what do I actually want to do , though? I want to make a chart showing early voters across counties - let's do the three most populated counties - Montgomery, PG and Baltimore County.

Soooo I need a data frame with those three counties as rows? and the affiliations as columns?
...
you decide to dig into the habits of Maryland political donors who give
money via ActBlue and WinRed.

You want to show Maryland's political geography - where those donors
live - and the frequency of donations. To do that, you'll load in two
CSV files: `md_winred_zip_totals.csv` and `md_actblue_zip_totals.csv`.

You also will need to get population by zip code for Maryland - we did a
similar thing in a previous lab, but this time you want the total
population as your variable. Use tidycensus to do that, and save that
dataframe, then join it to each of your contribution dataframes - you
will need to make sure that the zip codes you're joining on are the same
datatype.
