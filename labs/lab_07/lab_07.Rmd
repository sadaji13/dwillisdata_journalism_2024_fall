...
joined_frederick <- left_join(voting_locations,ballot_choices, by = "precinct")
joined_frederick
```
...
**A3**
- 11,871 voters split between the governor and comptroller.
- 95,221 voters voted for the same parties in both those races.
- About 11% of voters seem to have split their tickets.
- 


NOTE: Okay hang on those two numbers don't add up to 107,302. Why not?
SPOILER: CAUSE YOU DIDNT JOIN EARLIER
```{r}
ticket_splitters <- ballot_choices|> filter(governor_lt_governor=="DEM" & comptroller== "REP")

ticket_splitters2 <- ballot_choices|>filter(governor_lt_governor=="DEM" &comptroller=="LIB")

ticket_splitters3 <- ballot_choices|>filter(governor_lt_governor=="DEM" &comptroller=="GRE")

ticket_splitters4 <- ballot_choices|>filter(governor_lt_governor=="DEM" &comptroller=="WCP")

ticket_splitters5 <- ballot_choices|>filter(governor_lt_governor=="REP" &comptroller=="DEM")

ticket_splitters6 <- ballot_choices|>filter(governor_lt_governor=="REP" &comptroller=="LIB")

ticket_splitters7 <- ballot_choices|>filter(governor_lt_governor=="REP" &comptroller=="GRE")

ticket_splitters8 <- ballot_choices|>filter(governor_lt_governor=="REP" &comptroller=="WCP")
...
NOTE: REMEMBER != means not equal to
NOTE: Have to redo this part bc I forgot to join at the beginning
```{r}
ticketsplitters <- joined_frederick|> filter(governor_lt_governor != comptroller)

#SPLIT COMP AND GOV PERCENTAGE
(11871/107303)*100
```
```{r}
same_gov_and_comp <- joined_frederick|> filter(governor_lt_governor == comptroller)

#SAME COMP AND GOV PERCENTAGE
(95221/107303)*100
```
...
all_same <- ballot_choices|> filter(governor_lt_governor==comptroller, governor_lt_governor==u_s_senator, governor_lt_governor==representative_in_congress_6, governor_lt_governor==attorney_general)
...
It looks like 21702 was the zip code with the most voters who split just on the governor while voting republican on all the others.
```{r}
all_rep_but_gov <- joined_frederick|> filter(governor_lt_governor!="REP", comptroller=="REP", representative_in_congress_6=="REP", attorney_general=="REP", u_s_senator=="REP")|>
  group_by(zipcode)|>
  summarize(cast_vote_record = n())|>
