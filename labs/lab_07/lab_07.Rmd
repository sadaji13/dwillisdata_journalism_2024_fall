...
```{r}
acs5_geography <- load_variables(2022, "acs5/cprofile", cache = TRUE)
```
...
ticket_splitters5|>
  ...
  summarize(
    total_split_votes = sum(cast_vote_record))
...
```{r}
ticketsplitters <- ballot_choices|> filter(governor_lt_governor != comptroller)

```

```{r}
samesies <- ballot_choices|> filter(governor_lt_governor == comptroller)
samesies
```
