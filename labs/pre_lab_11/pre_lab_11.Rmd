...
Now the results are in the form of a data frame; both the candidates and the parties are in the column names; the 25th row has both the totals and the percentages 
...
The numbers represent each column position, allowing us to rename them without actually typing out the full name we want to change.
...
results <- results[[1]] |>
  ...
  mutate(sharon_harris_harris_write_in_democratic)
...
```
NOTE: html element will grab just the first, and html elements will grab ALL of the elelments
...
It's showing ALL characters, including white space and return characters
...
We've got all the URLS - the ones we do want, but also ones that begin with a backslash
...
#Filtering out urls that begin with a backslash - begin is indicated with the lil carrot
