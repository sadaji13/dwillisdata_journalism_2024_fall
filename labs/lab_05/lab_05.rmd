...
```
QUESTION: you can only join by one column, right??


Can you join by if multiple columns are the same??

```{r}
join_by_zip <- harris_combined_contribs|> left_join(new_voters, join_by(zip==ResidentialZip))
view(join_by_zip)
```
