...
```
```{r}
filtered <- md_voters_2024|> filter(STATE_REGISTRATION_DATE>"2024-07-21")
```
...
**Q1** How many voters appear to be *new* registrants since July 21, when President Joe Biden announced he would leave the race, based on the information above? Create a dataframe to hold only those voters. Which counties have seen the most? Take a look at county population figures here and compare the two in your answer: https://worldpopulationreview.com/us-counties/maryland.
...
```{r}
md_voters_2024 <- md_voters_2024 |> mutate(Week2024 = week(STATE_REGISTRATION_DATE))|>group_by(Week2024)|>summarize()
```
